 ...
 * loadSnapshotForDate both expect the file named for date X to hold X's
 ...
 * date's blob exists (so later calls that day are cheap and skip the
...
export async function saveTodaySnapshot(sheets, sources) {
  ...
  const pathname = pathFor(effectiveSnapshotDate());
