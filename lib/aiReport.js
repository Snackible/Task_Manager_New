...
import { parseSheetDate, toISODate, weekLabel, dayLabel, isoWeekStart } from "./dateUtils.js";
import { loadPreviousSnapshot } from "./snapshotStore.js";
import { diffTeamRows } from "./diffTasks.js";

const GEMINI_API_URL = "https://generativelanguage.googleapis.com/v1beta/models";
const MODEL = process.env.GEMINI_MODEL || "gemini-3.6-flash";

const BUCKET_LABEL = { pending: "Not Started", in_progress: "WIP", completed: "Done" };
...
- One "## TEAMNAME:" markdown heading (literally starting with "## ", team name in caps, colon) per team in
  ...
  Hold, Not Started) rather than as brackets or labels. End each team's section with an "Overall" bullet.
...
const PLAN_SYSTEM_PROMPT = `You are writing a Monday-morning plan for the week ahead, from a task tracker. Only
consider work that has been added on that week. completely ignore status of the work or previous tasks.


${STYLE_RULES}

Additional rules for this plan:
- Prioritize: call out the heaviest Not-Started backlogs first (they need work started), then WIP that
  looks stalled or heavy (may need focus to close out).
- End the whole report (after all teams) with a "**Focus this week:**" line naming the single highest-
  priority action across everything in scope.`;

const EOD_SYSTEM_PROMPT = `You are writing a short end-of-day recap from a task tracker, read at the close
of business. Focus on today's dated activity specifically; use the overall open backlog only as brief
context, not the main content. Mention any changes added via comments on the day report is generated.
...
export async function generateReport(payload, rawSheets, scope, type, week) {
  ...
  const prompt = await buildPrompt(payload, rawSheets, scope, week); // buildEodPrompt is async (reads a snapshot file); await works fine on the sync builders too
...
function reportDateLabel(payload, scope, type, week) {
  ...
  if (type === "plan") {
    return `Week of ${weekLabel(isoWeekStart(new Date()))}`;
  }
  // eod — the most recent day with dated activity, falling back to today
  const latestDay = latestDailyPeriod(payload, scope);
  return latestDay ? latestDay.label : dayLabel(new Date());
}

/** Most recent {periodStart, label, ...} daily bucket in scope, or null. */
function latestDailyPeriod(payload, scope) {
  ...
  return dailySource && dailySource.length ? dailySource[dailySource.length - 1] : null;
}
...
function extractTasks(rows, { buckets, onDate, dateRange } = {}) {
  const out = [];
  for (const row of rows || []) {
    ...
    out.push({ task, bucket, assignedTo, notes });
  }
  return out;
}
...
function formatTasks(tasks) {
  ...
  return tasks
    .map((t) => {
      ...
      return `  [${BUCKET_LABEL[t.bucket]}] ${t.task}${who}${note}`;
    })
    .join("\n");
}
...
function buildPlanPrompt(payload, rawSheets, scope) {
  const teams = scopeTeams(payload, scope);
  const latestDay = latestDailyPeriod(payload, scope);
  const onDate = latestDay ? latestDay.periodStart : null;

  const blocks = teams
    .map((team) => {
      if (team.source === "demo" || team.source === "empty") return teamBlock(team, rawSheets, {});
      const rows = (rawSheets[team.key] && rawSheets[team.key].rows) || [];
      const openTasks = extractTasks(rows, { buckets: ["pending", "in_progress"] });
      const closedToday = onDate ? extractTasks(rows, { buckets: ["completed"], onDate }) : [];
      return `${team.name.toUpperCase()}:\n${formatTasks(openTasks.concat(closedToday))}`;
    })
    .join("\n\n");

  return `Write the plan for the week ahead. Teams in scope: ${teams.map((t) => t.name).join(", ")}. Below is
each team's open work (WIP + Not Started, regardless of date), plus anything completed today${
    latestDay ? ` (${latestDay.label})` : ""
  } for context on recent momentum — everything else completed has already been excluded.

${blocks}`;
}

async function buildEodPrompt(payload, rawSheets, scope) {
  const teams = scopeTeams(payload, scope);
  const latestDay = latestDailyPeriod(payload, scope);
  const onDate = latestDay ? latestDay.periodStart : null;
  const dateLine = latestDay
  ...
  // The real "what changed today" signal: a diff against yesterday's saved
  // snapshot (see snapshotStore.js / diffTasks.js), rather than inferring
  // it from the current-state notes dump below. Falls back gracefully —
  // null on the first day this feature runs, before any snapshot exists.
  ...
  const blocks = teams
    .map((team) => {
      if (team.source === "demo" || team.source === "empty") return teamBlock(team, rawSheets, {});
      const rows = (rawSheets[team.key] && rawSheets[team.key].rows) || [];
      const todayTasks = onDate ? extractTasks(rows, { onDate }) : [];
      ...
      const openBacklog = extractTasks(rows, { buckets: ["pending", "in_progress"] });

      let changesBlock;
      if (!previous) {
        changesBlock = "  Changes since yesterday: not available yet (no snapshot has been captured before today).";
        ...
        changesBlock = changes.length
          ? `  Changes since ${previous.date} (this is the authoritative "what happened today" signal — a real diff, not a guess):\n${changes.map((c) => `    - ${c.task}: ${c.change}`).join("\n")}`
          : `  Changes since ${previous.date}: none detected.`;
      }

      return `${team.name.toUpperCase()}:
  Today's dated activity (${latestDay ? latestDay.label : "none"}):
${formatTasks(todayTasks)}
${changesBlock}
  Full open backlog (Not Started + WIP, regardless of date, for context):
