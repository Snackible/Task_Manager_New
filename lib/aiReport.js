...
A diff entry being "today's real activity" means the CHANGE happened today — not that the task itself is
new or from today. Every diff entry carries a "[dated: YYYY-MM-DD]" tag, the task's own received/due/closed
date. When that date is well before today, the task is an existing/ongoing item that merely got touched
today — report the change (it's real and worth mentioning), but frame it as an update to something already
in motion ("the ongoing follow-up with X, open since 24 Aug, had its notes updated to..."), never as if the
task itself started or is otherwise new today. Only a task whose OWN date matches today, or a genuinely
"new task added" diff entry, gets described as today's news outright.
...
- Do not backdate the open backlog. Every backlog line carries a "[dated: YYYY-MM-DD]" tag showing when THAT
  task was actually received/due/closed — often days or weeks before this report's date. Never describe a
  backlog-only item (one that's neither in "today's dated activity" nor the diff) using language that implies
  it happened today or was worked on today ("completed", "moved to", "closed out", "finished", "wrapped up",
  etc.) — that's exactly the mistake this tag exists to prevent. If a backlog item is worth mentioning at all,
  frame it explicitly as pre-existing/ongoing context, separate from today's own bullets (e.g. "separately,
  X has been open since 20 Aug and remains WIP") — never blended into the same bullet as today's activity as
  if it were part of it. When in doubt, leave it out; the backlog section is context for the reader, not
...
function extractTasks(rows, { buckets, onDate, dateRange, receivedSince } = {}) {
  ...
  for (const row of rows || []) {
    ...
    // Date Received column to check. Computed unconditionally (not just
    // when a date filter is active) so every task — including open-backlog
    // items with no date filter at all — carries a displayable date; see
    // formatTaskLines, which surfaces it so the model can tell a task
    // that's been open for weeks from one that's genuinely new today.
    const candidates = [
    ...
    if (onDate || dateRange || receivedSince) {
      // onDate ("did anything happen on day X" — EOD reports) checks EVERY
      // candidate, not just the first non-null one: a task received on day
      // D-1 but closed on day D (received/deadline outrank date-closed in
      // the priority order above) would otherwise anchor to D-1 and vanish
      // from D's report, even though closing it is the literal thing that
      // happened on D. dateRange/receivedSince (weekly reports, which want
      // one consistent anchor per task rather than "any field matches")
      // keep the single-priority anchor unchanged.
      if (onDate && !candidates.some((d) => toISODate(d) === onDate)) continue;
...
function formatTaskLines(tasks, showStatus, showDate) {
  ...
  // duplicates within THIS list and, where showDate isn't already putting a
  // date at the end of the line, fold the date into the title itself so
  ...
  return tasks
    .map((t) => {
      ...
      // Shown for the EOD "open backlog" list specifically — without a
      // visible date, the model has no way to tell a task that's been
      // sitting open for weeks from one that's genuinely new today, and
      // ends up narrating both the same way. See the "do not backdate"
      // rule in EOD_SYSTEM_PROMPT, which this date is what makes
      // enforceable.
      const dateTag = showDate ? ` [dated: ${t.anchorISO || "no date"}]` : "";
      const dupTag = !showDate && titleCounts[t.task] > 1 ? ` (${t.anchorISO || "undated"})` : "";
      return `  ${statusPrefix}${t.task}${dupTag}${who}${note}${dateTag}`;
    })
    .join("\n");
}

function formatTasks(tasks, { showStatus = true, showDate = false } = {}) {
  ...
  if (subTabs.length < 2) return formatTaskLines(tasks, showStatus, showDate);
  return subTabs
    .map(
      (sub) =>
        `  ${sub}:\n${formatTaskLines(tasks.filter((t) => t.subTab === sub), showStatus, showDate).replace(/^  /gm, "    ")}`
    )
    .join("\n");
}
...
async function buildEodPrompt(payload, rawSheets, scope, week, day) {
  ...
  if (isHistorical) {
    ...
    // when no snapshot was captured that day OR when storage itself is
    // unavailable — a DB outage should degrade the report (no historical
    // diff that day), never take down report generation entirely.
    daySnapshot = await loadSnapshotForDate(day).catch((err) => {
      console.error("[aiReport] loadSnapshotForDate failed, falling back to live data:", err);
      return null;
    });
  } else {
    const latestDay = latestDailyPeriod(payload, scope);
  ...
  // back gracefully — null before any snapshot exists yet, or if storage is
  // down (same reasoning as loadSnapshotForDate above: degrade, don't fail).
  const previous = await loadPreviousSnapshot(isHistorical ? day : undefined).catch((err) => {
    console.error("[aiReport] loadPreviousSnapshot failed, generating without a diff:", err);
    return null;
  });
  ...
  const blocks = teams
    .map((team) => {
      ...
      let rows = historicalRows || liveRows;
      if (isHistorical && historicalRows && onDate) {
        // A task dated for this exact day can still be incomplete in the
        // day's own snapshot — either missing entirely (logged after the
        // snapshot fired) or present but with a note/comment added later
        // (someone closes a task same-day but writes up the details the
        // next morning). Either way the snapshot permanently freezes the
        // stale version, and since a task's own date says it belongs to
        // this day regardless of when it was last edited, live data is the
        // MORE correct source for it — not "today's current state"
        // leaking in, but this day's own story finally complete. This is
        // narrower than trusting live data everywhere: it only overrides
        // the snapshot for rows LIVE data says are dated onDate; the
        // snapshot still wins for everything else (the open backlog particularly
        // — an old task that's since closed must still show as it stood
        // back then, which is exactly what the snapshot protects).
        const isOnDate = (r) => {
          const get = fieldGetter(r);
          const candidates = [
            parseSheetDate(get("date received", "date recieved")),
            parseSheetDate(get("deadline")),
            parseSheetDate(get("date closed")),
          ...
          return candidates.some((d) => toISODate(d) === onDate);
        };
        const liveOnDateBySig = new Map();
        for (const r of liveRows) {
          const sig = rowSignature(r);
          if (sig && isOnDate(r)) liveOnDateBySig.set(sig, r);
        }
        if (liveOnDateBySig.size) {
          const seen = new Set();
          rows = historicalRows.map((r) => {
            const sig = rowSignature(r);
            if (sig && liveOnDateBySig.has(sig)) {
              seen.add(sig);
              return liveOnDateBySig.get(sig); // live version — same task, possibly completed with a later note
            }
            return r;
          });
          for (const [sig, r] of liveOnDateBySig) {
            if (!seen.has(sig)) rows.push(r); // genuinely new to the snapshot
          }
        }
      }
      ...
      if (isHistorical && !historicalRows) {
        ...
        changesBlock = changes.length
          ? `  Changes since ${previous.date} (this is the authoritative "what happened${isHistorical ? " that day" : " today"}" signal — a real diff, not a guess. Each entry's [dated: ...] tag is the task's OWN date — received/due/closed — not ${onDate || "the report date"}; a change on a task dated well before ${onDate || "today"} means only its status/notes moved that day — the TASK ITSELF is old. Frame those as an update to an existing item, e.g. "the ongoing X task (from [date]) had its notes updated to...", never as if the task itself is new or started that day):\n${changes.map((c) => `    - ${c.task} [dated: ${c.dateISO || "no date"}]: ${c.change}`).join("\n")}`
...
${changesBlock}
  Full open backlog (Not Started + Awaiting Approval + WIP, as of ${isHistorical ? "that day" : "today"}, for context — each line's [dated: ...] tag is when THAT task was received/due/closed, NOT ${onDate || "the report date"}; do not backdate it):
${formatTasks(openBacklog, { showDate: true })}`;
    })
    .join("\n\n");
