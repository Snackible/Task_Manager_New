  ...
  const blocks = teams
    .map((team) => {
      ...
      if (isHistorical && historicalRows && onDate) {
        // A task dated for this exact day can still be missing from the
        // day's own snapshot if it was logged into the sheet sometime
        // after that snapshot was captured (people don't always log same-
        // day) — the snapshot then permanently misses it forever. Patch
        // that gap by pulling in any CURRENT live row dated onDate that
        // the snapshot doesn't already have (matched by title+Date
        // Received, same signature diffTasks.js uses), without touching
        // anything the snapshot already captured — an already-closed
        // task's since-changed status still shows its historical state,
        // not today's; only genuinely-missing same-day rows get added.
        const existingKeys = new Set(historicalRows.map(rowSignature).filter(Boolean));
        const lateAdditions = liveRows.filter((r) => {
          const sig = rowSignature(r);
          if (!sig || existingKeys.has(sig)) return false;
        ...
        if (lateAdditions.length) rows = [...historicalRows, ...lateAdditions];
      }
