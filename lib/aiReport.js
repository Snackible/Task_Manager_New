...
the complete filter. Completely ignore each task's current status (Done, WIP, On Hold, Not Started) when
deciding what to include or how to organize the write-up; status is shown per task for reference only, not
as a grouping or priority signal. This report answers "what's new since Monday," not "what's still open."

${STYLE_RULES}

Additional rules for this plan:
- Group by theme/initiative like usual, but organize around what's new and why it matters (deadline
  proximity, who owns it, priority level) rather than by status.
...
function extractTasks(rows, { buckets, onDate, dateRange, receivedSince } = {}) {
  ...
  for (const row of rows || []) {
    ...
    // Strictly Date Received, no fallback chain — "added this week" means
    // exactly that column, not whichever date happens to be set (a task
    // added months ago with this week's deadline is not "new").
    if (receivedSince) {
      const received = parseSheetDate(get("date received", "date recieved"));
      if (!received || received.getTime() < receivedSince.start.getTime() || received.getTime() > receivedSince.end.getTime()) continue;
    }
