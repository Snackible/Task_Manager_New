...
function assembleReport(teams, perTeam, chronicDelays, nonReporting, narrative, weekStartISO, weekEndISO) {
  ...
  for (const team of teams) {
    ...
    const sortedCarried = [...d.carriedOver].sort((a, b) => b.weeksPending - a.weeksPending);
    const shownCarried = sortedCarried.slice(0, MAX_CARRIED_TABLE_ROWS);
    const omittedCarried = sortedCarried.length - shownCarried.length;
    ...
    parts.push(
      carriedRows.length
        ? mdTable(["Item", "Owner", "First flagged (week of)", "Blocker", "Needs action from"], carriedRows)
        : "_Nothing carried over — clean slate._"
    );
    if (omittedCarried > 0) {
      parts.push(`\n_+ ${omittedCarried} more item(s) not shown — see the full backlog in the dashboard's task view._`);
    }
    ...
    if (n.accountabilityFlag) {
      ...
      if (chronicCount > 0) {
        parts.push(
          `\n**Accountability flag:** ${chronicCount} item(s) have been carried over for 2+ weeks — see the table above.`
        );
      }
    }
  }
